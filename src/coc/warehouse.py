        ...
        rows.append(
            {
                ...
                # v0.2 periodic-table facets (all optional)
                "system_kind": r.get("system_kind"),
                "substrate": r.get("substrate"),
                "origin": r.get("origin"),
                "boundary_clarity": r.get("boundary_clarity"),
                "primary_function": r.get("primary_function"),
                "lifecycle_stage": r.get("lifecycle_stage"),
                "main_feedbacks": json.dumps(r.get("main_feedbacks", []) or []),
                "dominant_constraints": json.dumps(r.get("dominant_constraints", []) or []),
                "emergent_properties": json.dumps(r.get("emergent_properties", []) or []),
                "failure_modes": json.dumps(r.get("failure_modes", []) or []),
                "primary_resources": json.dumps(r.get("primary_resources", []) or []),
                "canonical_examples": json.dumps(r.get("canonical_examples", []) or []),
